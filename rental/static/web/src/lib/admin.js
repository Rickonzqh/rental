  ...
  var events = admin.events = {
    ...
    ,serach: function(othis){
      othis.off('keypress').on('keypress',function(e){
        ...
        if(e.keyCode === 13){
          admin.req({
            url:'/evaluationmanage/getevaorder/'+this.value+'/',
            done:(res)=>{
              let data = res.data;
              admin.popup({
                title: '查询结果'
                ...
                , success: function (layero, index) {
                  ...
                  layui.view(this.id).render('result').done(function () {
                    $('#nscore').html('评测得分：'+data.score)
                    $('#ntime').html('评测时间：'+data.time)
                    $('#neval_id').html('评测编号：'+data.eval_id)
                    $('#nevaluations').html('评测名称：'+data.evaluations)
                    $('#nasset').html('关联项目：'+data.asset)
                    $('#nmanage').html('&nbsp&nbsp&nbsp&nbsp负责人：'+data.manage)
                    $('#ntelephone').html('联系电话：'+data.telephone)

                    $('#nevaluations').click(()=>{
                      location.hash = '/evaluationmanage/evaluationdetial/id=' + data.evaluations_id;
                    })
                    $('#nasset').click(()=>{
                      location.hash = '/assetmanage/rootdetial/id=' + data.asset_id;
                    })
                  });
                }
              });
